...
def parse_dataset_xml(xml_path):
    ...
    # Attempt to parse FASTA-style entries inside CustomSequence (e.g., >left_adapter, >right_adapter)
    custom_fasta = {}
    if custom_seq:
        name = None
        seq_lines = []
        for line in custom_seq.splitlines():
            line = line.strip()
            if not line:
                continue
            if line.startswith('>'):
                if name and seq_lines:
                    custom_fasta[name] = ''.join(seq_lines).replace(' ', '').replace('\t', '')
                name = line[1:].strip()
                seq_lines = []
            else:
    ...
    return {
        ...
        'adapters_primers': {
            ...
            'CustomSequence_fasta_keys': sorted(custom_fasta.keys()) if custom_fasta else [],
        },
        'barcodes': {
...
def write_outputs(xml_path, summary, write_decoded_barcodes=False, outdir=None):
    base = os.path.splitext(os.path.basename(xml_path))[0]
    outdir = outdir or os.path.dirname(xml_path)
    os.makedirs(outdir, exist_ok=True)
    out_json = os.path.join(outdir, base + '.summary.json')
    out_txt = os.path.join(outdir, base + '.summary.txt')
    ...
    if write_decoded_barcodes and summary['barcodes'].get('BarcodesFasta_decoded_preview'):
        ...
        barcodes_fasta = decode_b64(barcodes_b64) if barcodes_b64 else None
        if barcodes_fasta:
            out_fa = os.path.join(outdir, base + '.barcodes.fasta')
            with open(out_fa, 'w') as f:
                f.write(barcodes_fasta)

    # Write adapters FASTA if custom_fasta contains left/right entries
    cf = summary.get('custom_fasta') or {}
    if cf:
        out_adapt = os.path.join(outdir, base + '.adapters_from_custom.fasta')
        with open(out_adapt, 'w') as f:
            for k in ['left_adapter', 'right_adapter']:
                if k in cf and cf[k]:
                    f.write(f'>{k}\n')
                    f.write(cf[k].upper() + '\n')
...
def main():
    ...
    ap.add_argument('--write-barcodes-fasta', action='store_true', help='Also write decoded barcodes FASTA (in outdir)')
    ap.add_argument('--outdir', help='Directory to write outputs (defaults to directory of XML)')
    args = ap.parse_args()

    summary = parse_dataset_xml(args.xml)
    write_outputs(args.xml, summary, write_decoded_barcodes=args.write_barcodes_fasta, outdir=args.outdir)
    base = os.path.splitext(os.path.basename(args.xml))[0]
    outdir = args.outdir or os.path.dirname(args.xml)
    print('Wrote:', os.path.join(outdir, base + '.summary.json'))
    print('Wrote:', os.path.join(outdir, base + '.summary.txt'))
    if args.write_barcodes_fasta:
        print('Wrote:', os.path.join(outdir, base + '.barcodes.fasta'))
